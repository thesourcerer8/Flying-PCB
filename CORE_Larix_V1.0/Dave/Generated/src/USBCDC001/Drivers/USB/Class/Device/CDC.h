...
*/
			 ...
			 *  EVENT_USB_Device_ConfigurationChanged() event so that the endpoints are configured when the configuration containing
			 ...
			 *  be called frequently in the main program loop, before the master USB management task USB_USBTask().
			 *
			 *  \param[in,out] CDCInterfaceInfo  Pointer to a structure containing a CDC Class configuration and state.
			 */
			 ...
			 *  becomes full, or the CDC_Device_Flush() function is called to flush the pending data to the host. This allows
			 ...
			 *  the endpoint bank becomes full, or the CDC_Device_Flush() function is called to flush the pending data to
			 ...
			 *  CDC_Device_Flush() function is called to flush the pending data to the host. This allows for multiple bytes to be
			 ...
			 *  of bytes in the OUT endpoint bank only, and thus the number of calls to CDC_Device_ReceiveByte() which are guaranteed to
			 ...
			 *  returns a negative value. The CDC_Device_BytesReceived() function may be queried in advance to determine how many
